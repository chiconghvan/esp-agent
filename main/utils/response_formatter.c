                       ...
                       char *buffer, size_t buffer_size)
{
    ...
    int written = snprintf(buffer, buffer_size, "📋 <b>%s:</b>\n───────────────\n", label);
    ...
    if ((size_t)written < buffer_size - 64)
        snprintf(buffer + written, buffer_size - written, "───────────────\n📊 <i>Tổng cộng:</i> <b>%d task</b>", count);
                             ...
                             char *buffer, size_t buffer_size)
{
    ...
    for (int i = 0; i < count; i++) {
        ...
        char item[256];
        int item_len = snprintf(item, sizeof(item), "%d. %s <b>[#%" PRIu32 "] %s</b> %s\n", 
...
char *format_task_completed(const task_record_t *task, char *buffer, size_t buffer_size)
{
    ...
    int written = snprintf(buffer, buffer_size, "✅ <b>Đã hoàn thành:</b>\n");
    if (written > 0 && (size_t)written < buffer_size)
        written += append_task_detail(task, buffer + written, buffer_size - written);
    if (written > 0 && (size_t)written < buffer_size)
        snprintf(buffer + written, buffer_size - written, "\n✔️ <i>Xong lúc:</i> %s\n🎉 <b>Tốt lắm!</b>", completed_buf);
...
{
    int written = snprintf(buffer, buffer_size, "🗑️ <b>Đã hủy:</b>\n");
    if (written > 0 && (size_t)written < buffer_size)
...
{
    int written = snprintf(buffer, buffer_size, "📝 <b>Đã cập nhật:</b>\n");
    if (written > 0 && (size_t)written < buffer_size)
        written += append_task_detail(task, buffer + written, buffer_size - written);
    if (written > 0 && (size_t)written < buffer_size && changes != NULL)
        snprintf(buffer + written, buffer_size - written, "\n✏️ <i>Thay đổi:</i> %s", changes);
    dispatcher_set_context_tasks(&task->id, 1);
    return buffer;
}
...
char *format_reminder(const task_record_t *task, char *buffer, size_t buffer_size)
{
    ...
    if (written > 0 && (size_t)written < buffer_size)
        written += append_task_detail(task, buffer + written, buffer_size - written);
    if (written > 0 && (size_t)written < buffer_size)
        snprintf(buffer + written, buffer_size - written, "\n💡 <i>Trả lời 'xong' để đánh dấu hoàn thành</i>");
    return buffer;
}
